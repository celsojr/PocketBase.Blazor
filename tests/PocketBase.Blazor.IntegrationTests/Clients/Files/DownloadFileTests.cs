...
public class DonwloadTests
{
    ...
    {
        FileOptions options = new FileOptions { Thumb = "100x100" };
        Result<Stream> result = await _pb.Files.GetStreamAsync("_pb_users_auth_", "2xwhe4qnnegixob", "img_9126_3_e16nay7hie.jpg", options);

        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();

        string filePath = Path.Combine(Path.GetTempPath(), $"test_download_{Guid.NewGuid()}.jpg");
    ...
    {
        FileOptions options = new FileOptions { Thumb = "100x100" };
        Result<byte[]> result = await _pb.Files.GetBytesAsync("_pb_users_auth_", "2xwhe4qnnegixob", "img_9126_3_e16nay7hie.jpg", options);
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeNull();
